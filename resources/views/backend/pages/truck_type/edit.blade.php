<form id="editUserForm" action="{{ route('admin.truck.type.update') }}" method="post">
    ...
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Edit Truck Type</h5>
            ...
            <div class="col-sm-10">
                <select name="rent_type" class="form-control" id="rent_type"
                    ...
                    <option {{ $truck_details->truck_type->rent_type == 'distance' ? 'selected' : '' }}
                        value="distance">Rent based on distance</option>
                    <option {{ $truck_details->truck_type->rent_type == 'load' ? 'selected' : '' }} value="load">Rent
                        based on load</option>
                </select>
            </div>
        </div>
                ...
                @if ($truck_details->truck_type->rent_type == 'distance')
                    <div class="col-sm-10">
                        <input type="number" name="rent_amount[]" value="{{ $truck_details->rent_amount }}"
                            class="form-control" placeholder="Rent amount" required>
                    </div>
                @endif
                @if ($truck_details->truck_type->rent_type == 'load')
                    <div class="col-sm-10 d-flex gap-2">
                        <input type="text" name="load_type" value="{{ $truck_details->load_type }}" class="form-control" placeholder="Load type" required>
                        <input type="number" name="rent_amount" value="{{ $truck_details->rent_amount }}" class="form-control" placeholder="Rent amount"
                            required>
                    </div>
                @endif
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <input type="number" name="driver_charge" value="{{ $truck_details->truck_type->driver_charge }}"
                    class="form-control" placeholder="Driver charge" required>
            </div>
        </div>
        <div class="form-group  row">
